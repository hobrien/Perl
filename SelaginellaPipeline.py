...
merg_info = path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Mappings", species + "_Tr_cons_unstranded_merge.txt")
...
log.write("ParallelBatch.py -f %s -f .fa -c 'RunPhyml.py * fast'\n" % contig_folder)
system("ParallelBatch.py -f %s -f .fa -c 'RunPhyml.py * fast'" % contig_folder)
...
log.write("ParallelBatch -p 1 -f %s -f .fa -c 'RunMakeConsensus.py * %s'\n" % (contig_folder, species) )
system("ParallelBatch -p 1 -f %s -f .fa -c 'RunMakeConsensus.py * %s'" % (contig_folder, species) )

#Index consensus file for read mapping
log.write("\n%s, Index consensus file for read mapping:\n" % time.asctime(time.localtime()))

log.write("cp %s %s\n" % (consensus_file, index_file))
system("cp %s %s" % (consensus_file, index_file))

log.write("bowtie2-build %s %s\n" % (indexed_file, index))
system("bowtie2-build %s %s" % (indexed_file, index))

#Run TopHat mapping and cufflinks on each library
for x in rage(lib_num):
  x = x + 1
  
  mapping_dir = path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Mappings", species + x + "_Tr_cons_unstranded")
  f_reads = ",".join((path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Filtered", species + x + "_1.filtered.fastq"), 
                      path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Filtered", species + x + "b_1.filtered.fastq")
                    ))
  r_reads = ",".join((path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Filtered", species + x + "_2.filtered.fastq"), 
                      path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Filtered", species + x + "b_2.filtered.fastq")
                    ))
  ...
  bam_file = path.join(path.expanduser("~"), "Bioinformatics", "Selaginella", "Mappings", species + "_Tr_cons_unstranded", species + x + "Tr_cons.bam")
  ...
  log.write("mv %s %s" % (read_folder + "/accepted_hits.bam\n", bam_file))
  system("mv %s %s" % (read_folder + "/accepted_hits.bam", bam_file))
  
  log.write("samtools index %s\n" % bam_file)
  system("samtools index %s" % bam_file)

  log.write("\n%s, Run cufflinks of %s%s read mapping:\n" % (time.asctime(time.localtime()), species, x))  
  log.write("cufflinks -o %s %s\n" % ( cufflinks_file, bam_file))
  system("cufflinks -o %s %s" % ( cufflinks_file, bam_file))
  
  mergeinfo_fh = open(merge_info, "a")
  mergeinfo_fh.write(cufflinks_gtf)
  mergeinfo_fh.close()
  
#Run cuffmerge on transcripts
log.write("\n%s, Run cuffmerge on cufflinks transcripts:\n" % time.asctime(time.localtime()))
log.write("cuffmerge -o %s -s %s %s\n" % (merge_dir, consensus_file, merg_info)) 
system("cuffmerge -o %s -s %s %s" % (merge_dir, consensus_file, merg_info))
